...
module.exports = {
    ...
    getYearMovie: function(req, res){
        Movie.where("year").lte(parseInt(req.params.year1)
        ).gte(parseInt(req.params.year2)).exec(function(err, movies){
            if(err) return res.status(500).json(err);
            res.json(movies);
        });
    },
    deleteBeforeYear: function(req, res){
        let yearLimit = req.params.deleteYear;
        Movie.deleteMany({year: {$lt: yearLimit}}, function(err, movies){
            if(err) return res.status(500).json(err);
            res.json({
